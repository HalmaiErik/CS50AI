...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.mark_safe(cell)
        self.add_sentence_for_neighbors(cell, count)
        self.update_mines_and_safes()
        self.infer_from_knowledge()
             
    def add_sentence_for_neighbors(self, cell, count):
        sentence_cells = set()
        start_row = max(cell[0] - 1, 0)
        end_row = min(cell[0] + 2, self.height)
        start_col = max(cell[1] - 1, 0)
        end_col = min(cell[1] + 2, self.width)
        ...
        for i in range(len(self.knowledge)):
            for j in range(i + 1, len(self.knowledge)):
                ...
                if sentence_i.cells.issubset(sentence_j.cells):
                    self.add_subset_sentence(sentence_i, sentence_j)
                elif sentence_j.cells.issubset(sentence_i.cells):
                    self.add_subset_sentence(sentence_j, sentence_i)
                    
    def add_subset_sentence(self, sub_sentence, sentence):
        new_sentence = Sentence(sentence.cells - sub_sentence.cells, sentence.count - sub_sentence.count)
